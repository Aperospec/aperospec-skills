...
Use this template only when the input is a complete Cognitive World Package (CWP) from `project`.
...
They are narrative scene potentials for `conceptdeck` to later translate into a Concept Deck Package (CDP).
```
...
- The NWP can be passed directly to `conceptdeck`.
